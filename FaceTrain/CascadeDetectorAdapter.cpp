...
//    String path = "/Users/zcw/dev/live_lib/david/result/cascade.xml";
    String path = "/Users/zcw/dev/live_lib/opencv/data/lbpcascades/lbpcascade_frontalface.xml";
    /**
   *   显示摄像头的图像
   */
    ...
    if(!capture.isOpened()) {
        printf("can not open VideoCapture  get capture\n");

    } else {
        printf("open VideoCapture  get capture\n");
    ...
    while (1) {
        ...
        for (Rect face : faces) {
            ...
            char p[100];
            sprintf(p, "/Users/zcw/dev/live_lib/david/%d.jpg", i++);
            //把mat写出为jpg文件
            imwrite(p, m);
            m.release();
#endif
